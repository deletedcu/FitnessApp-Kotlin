 ...
 */
class DashboardWorkoutTypeAdapter(val items: List<WorkoutType>, val size: Int, val listener: (View, WorkoutType) -> Unit) : RecyclerView.Adapter<DashboardWorkoutTypeAdapter.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = ViewHolder(parent.inflate(R.layout.fragment_dashboard_workout_type_card), size)

    override fun onBindViewHolder(holder: ViewHolder, position: Int) = holder.bind(items[position], listener)

    override fun getItemCount() = items.size

    class ViewHolder(itemView: View, size: Int) : RecyclerView.ViewHolder(itemView) {
