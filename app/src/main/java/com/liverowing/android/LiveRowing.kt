...
import com.liverowing.android.service.device.BleDevice
import com.liverowing.android.service.device.Device
import com.parse.*
...
class LiveRowing : Application() {
    ...
    fun onDeviceConnectRequest(message: DeviceConnectRequest) {
        ...
        if (device is BluetoothDevice) {
            ...
            com.liverowing.android.service.device.usb.UsbDevice(this@LiveRowing, device).apply {
                connect()
            }
        }
    }
    ...
    companion object {
        ...
        fun globalParseExceptionHandler(activity: Activity, exception: ParseException) {
            when (exception.code) {
                ...
                else -> Log.d("LiveRowing", "Unhandled parse exception: ${exception.code}: ${exception.message}")
            }
        }
    }
}
