...
import com.liverowing.android.LiveRowing.Companion.deviceConnected
...
import com.liverowing.android.service.messages.WorkoutProgramRequest
import com.liverowing.android.service.messages.WorkoutProgrammed
import com.liverowing.android.service.messages.WorkoutSetup
import com.liverowing.android.util.WorkoutRecorder
...
import com.liverowing.android.view.SplitIntervalOverviewView
...
import com.liverowing.android.view.TextMetricView
...
class RaceActivity : AppCompatActivity(), WorkoutRecorder.Callback {
    ...
    override fun onResume() {
        ...
        if (!deviceConnected) {
            ...
            EventBus.getDefault().post(WorkoutProgramRequest(mWorkoutType, mTargetPace))
        }
    }
    ...
    fun onWorkoutSetup(setup: WorkoutSetup) {
        ...
        val secondaryMetricClickListener = View.OnClickListener { view ->
            ...
            race_racing_stroke_ratio.visibility = if (race_racing_metric_center.metricId == Metric.SECONDARY_METRIC_STROKE_RATIO) View.VISIBLE else View.GONE
            updateMetrics(true)
        }
    ...
    private fun getPositions(index: Int, me: RowingStatus, pb: Progress, opponent: Progress): Positions {
        return when (mWorkoutType.valueType) {
            ...
            VALUE_TYPE_TIMED -> {
                ...
                val previousSplitDistance = getDistanceInPreviousSplits(mWorkoutRecorder.splits, currentSplit)
                val mySplitDistance = me.distance - previousSplitDistance
                val ratio = splitTime / time
                val probableDistanceInSplit = ratio * mySplitDistance
                val personalBestPosition = if (pb.dataPoint is Workout.DataPoint) getPositionByDistance(currentSplit, probableDistanceInSplit, pb.dataPoint.meters) else 0f
                val opponentPosition = if (opponent.dataPoint is Workout.DataPoint) getPositionByDistance(currentSplit, probableDistanceInSplit, opponent.dataPoint.meters) else 0f

                Positions(index, time.toFloat(), (time / splitTime).toFloat(), personalBestPosition, opponentPosition)
            }
    ...
    private fun getDistanceInPreviousSplits(splits: List<Workout.Split>, index: Int): Double {
        return splits.sumByDouble { if (it.splitNumber < index) it.splitDistance else 0.0 }
    }
